...
for kitchens in range(1,11):
    for waiters in range(1,11):

        # Obtenha o tempo de início
        tempo_inicial = time.time()
        
        result = subprocess.run(["time", "-p", "./restaurant-fixed.x", "20", f"{kitchens}", f"{waiters}"] , capture_output=True).stderr
        result = result.decode("utf-8")
        # Obtenha o tempo de término
        tempo_final = time.time()
        ...
        with open("results-paralelo-fixed.txt", "a+") as result_file:
            result_file.write(f"{kitchens},{waiters},{real_time},{user_time},{sys_time}\n")
...
for _ in range(1):
    ...
    result = subprocess.run([f"time", "-p", "./restaurant-serial-fixed.x", "20"] , capture_output=True).stderr
    result = result.decode("utf-8")
    # Obtenha o tempo de término
    tempo_final = time.time()
